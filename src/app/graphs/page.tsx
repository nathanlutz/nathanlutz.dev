    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      {graphs.length === 0 ? (
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          No graphs yet. Run a script in <code>graphs/</code> to generate one.
        </p>
      ) : (
        <div className="space-y-3 sm:space-y-0 sm:overflow-hidden sm:rounded-[1.5rem] sm:border sm:border-zinc-200 sm:dark:border-zinc-800">
          {graphs.map((graph) => {
            const last = graph.frames[graph.frames.length - 1];
            const span = `${graph.frames[0].year}–${last.year}`;
            ...
            return (
              ...
              >
                ...
                <div className="h-28 w-full px-4 py-2 sm:h-14 sm:rounded-xl sm:border sm:border-zinc-200 sm:px-1 sm:py-1 sm:dark:border-zinc-800">
                  <GraphChart
                    axes={graph.axes}
                    series={graph.series}
                    points={last.points}
                    yMax={last.yMax ?? graph.axes.y.domain[1]}
                    variant="thumb"
                  />
                </div>
                ...
                <div className="meta hidden text-zinc-400 sm:block sm:text-right">
                  {span}
                </div>
              </Link>
            );
